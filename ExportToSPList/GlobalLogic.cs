    ...
    class GlobalLogic
    {
        ...
        public Boolean createList(ClientContext ctx, string appSettingsKey, int templateType)
        {
            ...
            try
            {
                ...
                ctx.Load(field);
                ctx.ExecuteQuery();
                Console.WriteLine("Successfully created list:- " + appSettingsKey);
                success = true;
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
                Console.WriteLine("Failed to created list:- " + appSettingsKey);
                success = false;
            }
            return success;
            //return getListItemsCollection;
        ...
        public FieldLookupValue GetLookupFieldValueId(ClientContext clientContext, string value, string lookupListName, string lookupFieldName, string lookupFieldType, string Idvalue, string lookupFieldIdName, string lookupFieldIdType, GlobalLogic gl, string listName)
        {
            List list = null;
                                    ...
                                    <Query>
                                        <Where>
                                            <And>
                                                ...
                                                <Eq>
                                                    <FieldRef Name='{3}'></FieldRef>
                                                    <Value Type='{4}'>{5}</Value>
                                                </Eq>	
                                            </And>
                                        </Where>
                                    </Query>
                                </View>", lookupFieldName, lookupFieldType, value, lookupFieldIdName, lookupFieldIdType, Idvalue);
...
//                                </View>", lookupFieldName, lookupFieldType, value);
                    ...
                    if (listItems != null)
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            Console.WriteLine(" \n");
                            Console.WriteLine("GetHashCode = {0}", e.GetHashCode());
                            Console.WriteLine(" \n");
        ...
        #region bkp GetLookupFieldValue
        public FieldLookupValue GetLookupFieldValue(ClientContext clientContext, string value, string lookupListName, string lookupFieldName, string lookupFieldType)
        {
            List list = null;
            FieldLookupValue lookupValue = null;
            ...
            try
            {
                ...
                if (list != null)
                {
                    ...
                    if (listItems != null)
                    {
                        ...
                        {
                            Console.WriteLine(e.Message);
                            ExceptionHandle(e, lookupFieldName + "-----" + value);
                            lookupValue = new FieldLookupValue();
                            lookupValue.LookupId = 1000000;
                        }
                    }
                    //else
                    //{
        ...
        //public static void ExceptionHandle(Exception e, GlobalLogic gl, string additionalDetails)
        public static void ExceptionHandle(Exception e, string additionalDetails)
        {
            try
            {
                GlobalLogic gl = new GlobalLogic();
                ...
                xlWorkSheet.Cells[1, 1] = "Exception Message :- ";
        ...
        public void addWorkflowSubscription(ClientContext clientContext, string listname, Guid targetListGuid)
        {
            try
            {
                //    <add key="workflowHistoryListID" value="334FB71F-573A-40DB-A45B-05228BE1499D" />
                //<add key="taskListID" value="D631230F-23C1-41FA-AE02-7714BE79ED0D" />

                //Name of the SharePoint2013 List Workflow
                //string workflowName = "SplitTask WOrkflow";

                //string workflowName = "DelayNotification";
                string workflowName = ConfigurationManager.AppSettings.Get("workflowName");
                ...
                //<add key="workflowHistoryListID" value="8702CABA-4F37-422B-9206-6520476919BF" />
                //<add key="taskListID" value="E65B879A-B430-4344-9A89-F756B27B34B4" />
            ...
            try
            {
                ...
                for (int i = 0; i < dataSQL.Rows.Count; i++)
                {
                    ...
                    if (drExists != null && drExists.Length > 0)
                    {
                        ...
                        //DataRow drExists2 = (from row in dataSP.AsEnumerable() where row.Field<int>("ID") == 1 select row).SingleOrDefault();
